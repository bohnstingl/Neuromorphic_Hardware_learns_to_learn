...
"""This module provides functionality to use pybrain_local with rlglue and to use it
...
from pybrain_local.structure.modules.module import Module
from pybrain_local.rl.agents import LearningAgent
from pybrain_local.utilities import threaded
from pybrain_local.tools.benchmark import BenchmarkDataSet
...
def adaptAgent(agent_klass):
    """Return a factory function that instantiates a pybrain_local agent and adapts
    it to the rlglue framework interface.
    
    :type   agent_klass:    subclass of some pybrain_local agent
...
def adaptAgentObject(agent_object):
    """Return an object that adapts a pybrain_local agent to the rlglue interface.
    """
...
class RlglueAgentAdapter(object):
    """Wrapper class to use pybrain_local agents with the RLGlue library."""
    ...
    def __init__(self, klass, *args, **kwargs):
        ...
        :type   klass:    subclass of some pybrain_local agent
    ...
    def agent_init(self, task_specification=None):
        ...
        Since pybrain_local agents are not using task specifications
    ...
    def _integrateObservation(self, observation):
        ...
        it into the pybrain_local agent.
...
class RlCompBenchmark(object):
    """Class to run benchmarks of pybrain_local agents on the rl-competition 2007
    environments.
    """
...
# pybrain_local environments, other ways should be found.
