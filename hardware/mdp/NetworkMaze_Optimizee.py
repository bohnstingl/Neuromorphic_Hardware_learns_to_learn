...
class DLSMazeOptimizee(Optimizee):
    
    def __init__(self, traj=None):
        
        super(DLSMazeOptimizee, self).__init__(traj)
    ...
    def create_individual(self):
        ...
        return {'gamma': np.float64(np.random.random()),
              ...
              'exitatory' : np.float64(np.random.uniform(0., 1.)),
              'inhibitory' : np.float64(np.random.random())}
    ...
    def bounding_func(self, individual):
        ...
        return {'gamma': np.float64(np.clip(individual['gamma'], a_min=0., a_max=1.)),
                ...
                'inhibitory' : np.float64(np.clip(individual['inhibitory'], a_min=0., a_max=1.))}
    ...
    def simulate(self, traj):
        ...
        for i in range(20):
            ...
            self.network.Run(gamma, lam, eta, maxIteration, hdf=False, use32BitParams=use32BitParams, weightLower=weightLower, weightUpper=weightUpper, rescaleFreq=rescaleFreq, verbose=False, exitatory=exitatory, inhibitory=inhibitory)
