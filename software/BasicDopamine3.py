...
actionNeurons = nest.Create('iaf_psc_exp', nActions)
excitatoryNoise = nest.Create('poisson_generator', nActions)
...
nest.Connect([parrotNeuron[1]], [actionNeurons[0]],
             {'rule': 'one_to_one'},
             { 'model': 'layer1_stdp_synapse4'})
nest.Connect([parrotNeuron[1]], [actionNeurons[1]],
             {'rule': 'one_to_one'},
             { 'model': 'layer1_stdp_synapse5'})
nest.Connect([parrotNeuron[1]], [actionNeurons[2]],
             {'rule': 'one_to_one'},
...
for trials in range(Nrep):
    ...
    if len(events['times']) != 0:
        ...
        #ipdb.set_trace()P
        nest.SetStatus(rewardNeuron, {'spike_times' : []})
        nest.SetStatus(rewardNeuron[(action-1) * nRewardNeuronsPerAction:action*nRewardNeuronsPerAction], {'spike_times' : np.arange(20.0, 80.0, np.around(2. / (reward + 1.), decimals=1))})
...
ql = mdptoolbox_local.mdp.QLearning(P, R, 0.96)
ql.run()
#print(ql.Q)
print(ql.policy)