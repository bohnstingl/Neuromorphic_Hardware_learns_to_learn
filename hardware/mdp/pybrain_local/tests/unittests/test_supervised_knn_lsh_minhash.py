...
Internal Tests:
    ...
    >>> from pybrain_local.supervised.knn.lsh.minhash import arrayPermutation
    >>> permutation = array([4, 3, 2, 1, 0])
    >>> permute = arrayPermutation(permutation)
    >>> permute(array([5, 2, 3, 1, 4]))
    array([4, 1, 3, 2, 5])

    >>> from pybrain_local.supervised.knn.lsh.minhash import jacardCoefficient
    >>> a = array([0, 0, 0, 1])
    >>> b = array([1, 1, 1, 1])
    >>> c = array([1, 1, 0, 1])
    >>> d = array([0, 0, 1, 1])
...
Example Usage:
    ...
    >>> from pybrain_local.supervised.knn.lsh.minhash import MinHash
