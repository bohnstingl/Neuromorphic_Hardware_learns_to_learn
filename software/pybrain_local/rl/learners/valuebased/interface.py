...
from pybrain_local.structure.modules import Table, Module
...
from pybrain_local.utilities import one_to_n
...
class ActionValueTable(Table, ActionValueInterface):
    ...
    def getMaxAction(self, state):
        ...
        return argmax(self.params.reshape(self.numRows, self.numColumns)[int(state), :].flatten())

    def getActionValues(self, state):
        return self.params.reshape(self.numRows, self.numColumns)[int(state), :].flatten()
