...
from bandit_optimizee import BanditOptimizee, BanditParameters
from snn_mab import IncrementalLearningRule, Connector
...
def main(dependent, optimizer):
    ...
    name = 'MAB_Q_{}_{}_{}'.format(identifier, opt, dep_name)
    ...
    with open(os.path.join('calibrations', board_calibration_map[allocated_id]['cap'])) as f:
        calibrated_config = json.load(f)
    with open(os.path.join('calibrations', board_calibration_map[allocated_id]['dac'])) as f:
    ...
    optimizee_parameters = \
        BanditParameters(n_arms=2, n_pulls=100, n_samples=40, seed=optimizee_seed,
                         max_learning_rate=max_learning_rate, learning_rule=IncrementalLearningRule,
    ...
    if opt == 'CE':
        ce_optimizer_parameters = CrossEntropyParameters(
            pop_size=pop_size, rho=0.15, smoothing=0.2, temp_decay=0,
            n_iteration=n_iteration,
            distribution=NoisyGaussian(noise_magnitude=1., noise_decay=.73),
